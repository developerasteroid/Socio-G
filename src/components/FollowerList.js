...
export default function FollowerList({navigation, route}) {
    ...
    useEffect(()=>{
        const fetchFollowerList = async(userId="") => {
            ...
            try {
                const response = await axiosInstance.get(`api/user/get/followers/${userId}`);
                if(response.data){
                    setFollowerUserData(response.data);
        ...
        if(route.params && route.params.uid){
            fetchFollowerList(route.params.uid);
        } else {
            fetchFollowerList();
        }
    ...
    const removeFollower = async(targetId) => {
        const removedFollowerFilteredList = filteredUserData.filter(user => user.uid !== targetId);
        setFilteredUserData(removedFollowerFilteredList); 
        const removedFollowerList = followerUserData.filter(user => user.uid !== targetId);
        setFollowerUserData(removedFollowerList);

        try {
            const response = await axiosInstance.post('api/user/follower/remove', {targetUserId:targetId});
        } catch (error) {
            if(error.response && error.response.data && error.response.data.message){
            ...
            <TouchableOpacity style={styles.userContainer} onPress={()=>{navigation.push('UserProfile', {key: getRandomUUID(), uid:item.uid})}}>
                <View style={styles.profileCntainer}>
                    ...
                    <View style={styles.removeBtnContainer}>
                        <TouchableOpacity style={styles.removeBtn} onPress={()=>{removeFollower(item.uid)}}>
    ...
    const handleSearch = async(text) => {
        if(!text){
            setFilteredUserData(followerUserData);
            return;
        }
        const filteredList = followerUserData.filter(user => user.username.includes(text.toLowerCase()));
